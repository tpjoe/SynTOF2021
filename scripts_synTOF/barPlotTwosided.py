...
# # import data ------------------------------------------------------------
# df = pd.read_csv('../raw_data/Toms_mean_intensity_change.csv')
# df.Mean = df.Mean.apply(lambda x: re.sub(",", "", x)).astype('float')
# df.SEM = df.SEM.apply(lambda x: re.sub(",", "", x)).astype('float')



# df_human_pre = df.loc[(df.Group!='PSAPP') & (df.PrePost=='Pre'), :]





# XX = df_human_pre.loc[:, ['Mean', 'SEM', 'Marker', 'Group']]
...
# ax2.set_xlim(0,50)
# ax1.set_xlim(16000, 20000)
# ax1.set_xticks(np.arange(15000, 20001, 2000))
# # bars1 = XX.plot(ax=ax1, kind='barh')
# # bars2 = XX.plot(ax=ax2, kind='barh')
# sns.barplot(ax=ax2,
#     data=XX, x="Mean", y="Marker", hue="Group", ci='SEM',
#     palette=['black', '#fa164f'], capsize=.2)
