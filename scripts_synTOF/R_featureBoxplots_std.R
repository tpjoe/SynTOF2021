...
for (region in c('BA9', 'DLCau', 'Hipp')) {
    ...
    df_ <- read.csv(paste0('R_py_exchange/df_meanAllMarkers_', region, '_Std_exp2mc_5_13.csv'))[, -1]
    if (region != 'BA9') {
...
for (Pro in c('DJ1')) {#}, 'PHF-tau')) {
    ...
    ggsave(filename=paste0('figures/boxplots/All/expression_', Pro, '_Std_exp2mc_5_13', '.pdf'), p, width=10.5, height=16)
...
for (Pro in c('ApoE')) {
    ...
    p <- ggboxplot(data, x='resilient', y='Expression', color='black', facet.by='Marker', outlier.shape=NA,
            short.panel.labs=TRUE, scales="free", strip.position="top", ncol=5) + #, add="jitter") +
            geom_jitter(aes(colour=group), size=3.4, position=position_jitter(0.15)) +
            theme_classic() + labs(x="", y="Mean Expression") +
            scale_color_manual(values=c("black", "#fa164f", "#2a18f2")) + #, 'orange')) + 
            theme(legend.position="top", legend.title = element_blank()) +
            stat_compare_means(
...
for (selectedFeature in desired_feature) {
    ...
    data <- df_sel %>% pivot_longer(-group, names_to="Marker", values_to="Expression")
    data <- as.data.frame(data)
    data$group <- ordered(factor(data$group), levels=c('Control', 'AD', 'LBD'))
    data$Marker <- factor(data$Marker, levels=unique(data$Marker), ordered=TRUE)

    # faceted boxplots
    p <- ggboxplot(data, x='group', y='Expression', color='group', outlier.shape=NA, fill='group', alpha=0.2,
            ...
            scale_fill_manual(values=c("black", "#fa164f", "#2a18f2"))  +
            ...
            stat_compare_means(
                mapping=aes(label=format.pval(..p.., digits=1)),
                method="wilcox.test", ref.group="Control", cex=8,
                label.y.npc=0.9) + ylim(0, 0.5) +
...
for (selectedFeature in desired_feature) {
    ...
    p <- ggboxplot(data, x='group_resilience', y='Expression', color='group_resilience', outlier.shape=NA, fill='group_resilience', alpha=0.2,
            short.panel.labs=TRUE, scales="free", strip.position="top") +
            geom_jitter(aes(colour=group_resilience), size=4, position=position_jitter(0.15)) +
            theme_classic() + labs(x="", y="Mean Expression") +
            scale_color_manual(values=c("black", rgb(120/237, 77/237, 43/237), rgb(126/237, 33/237, 219/237)))  +
            scale_fill_manual(values=c("black", rgb(120/237, 77/237, 43/237), rgb(126/237, 33/237, 219/237)))  +
            theme(legend.position="none", plot.title=element_text(hjust=0.5, size=20), text=element_text(size=20.5), 
                  axis.text=element_text(size=19), axis.text.x=element_text(size=14)) +
            stat_compare_means(
                comparisons = my_comparisons,
                mapping=aes(label=format.pval(..p.., digits=1)),
                method="wilcox.test", cex=9, size=6, 
                label.y.npc=0.9) +
            theme(plot.margin = unit(c(0.2, 0.2, 0.2, 0.2), "cm")) 
    p <- p + labs(title=selectedFeature) + scale_x_discrete(labels=c('Permissive AD'='Permissive\nAD', 'Resilient AD'='Resilient\nAD'))
    ggsave(filename=paste0('figures/boxplots/AD_res/singleExpression_', selectedFeature, '.pdf'), p, width=4, height=6)
}
...
for (selectedFeature in desired_feature) {
    ...
    p <- ggboxplot(data, x='group_resilience', y='Expression', color='group_resilience', outlier.shape=NA, fill='group_resilience', alpha=0.2,
            ...
            theme(plot.margin = unit(c(0.2, 0.2, 0.2, 0.2), "cm")) 
    p <- p + labs(title=selectedFeature) + scale_x_discrete(labels=c('Permissive AD'='Permissive\nAD', 'Resilient AD'='Resilient\nAD'))
    ggsave(filename=paste0('figures/boxplots/AD_resGFAPnegEAAT1neg/singleExpression_', selectedFeature, '.pdf'), p, width=4, height=6)
}
