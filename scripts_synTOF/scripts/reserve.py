...
def pretrain(files, identifier, dims, i):
    ...
    # opt = tf.keras.optimizers.Nadam(learning_rate=1E-15)
    ...
    ae1.fit(x=x_train, y=x_train, batch_size=2**13, epochs=5000, callbacks=[cb])
    ae3.fit(x=x_train, y=x_train, batch_size=2**13, epochs=5000, callbacks=[cb])
    save_dir = '../results_ae'
...
def fit_predict(files, identifier, dims, n_clusters_list, i):
    ...
    merged_hidden = concatenate([ae1.get_layer(name='encoder_' + '02').output,
                                ae3.get_layer(name='encoder_' + '22').output])
    ...
    megaAE.compile(loss={'clustering': 'kld', 
                ...
                optimizer='Nadam')
    cl = clustering2K(model=megaAE, encoder=encoder, x=x_train, n_clusters=n_clusters, tol=0.03, batch_size=2**9)
...
def automated_cluster(files, identifier, dims):
    ...
    for i in range(len(glob(save_dir + '/ae1_' + identifier +'_*'))):
        ...
        n_clusters.append(int(get_cluster_num(h, maxK=30, subsampling_n=10000, 
...
identifier = region + 'test'
# dims = [[56, 28, 6], [56, 28, 3]]
...
to_R = pd.concat([cl_pred, pd.DataFrame(sample_pred[0][0:100000]).rename(columns={0:'sample'})], axis=1)
...
to_R = pd.concat([hidden_, pd.DataFrame(sample_pred[0][0:100000]).rename(columns={0:'sample'})], axis=1)
