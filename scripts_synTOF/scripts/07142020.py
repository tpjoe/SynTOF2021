...
def pretrain(x_train, identifier, dims, i):
    ...
    ae3.fit(x=x_train_scaled, y=x_train_scaled, batch_size=2**15, epochs=5000, callbacks=[cb])
    save_dir = '../results_ae'
...
def predict_(identifier, x_train, i):
    ...
    q, _, _ = megaAE.predict([x_train, x_train_scaled])
    cl_pred = q.argmax(1)
...
def fit_predict(x_train, identifier, dims, n_clusters_list, i):
    ...
    # import other scripts
    from importlib import reload
    from utils_test import clustering2, ClusteringLayer, get_cluster_num, clustering2K, clustering2K_std
    import utils_test
    # for reproducibility
    disabling_blas()
    from sklearn import preprocessing
    ...
    megaAE.compile(loss={'clustering': 'kld', 
                   ...
                   optimizer='Nadam')
    cl = clustering2K_std(model=megaAE, encoder=encoder, x1=x_train, x2=x_train_scaled, n_clusters=n_clusters, tol=0.03, batch_size=2**15)
...
def automated_cluster(x_train, identifier, dims):
    ...
    for i in range(len(glob(save_dir + '/ae1_' + identifier +'_*'))):
        ...
        h = get_output([x_train, x_train_scaled])[0]
...
identifier = region + 'LowNo_07142020Batch215_105_noStd'
# dims = [[56, 28, 28, 6], [56, 28, 28, 2]]
dims = [[512, 256, 128, 10], [512, 256, 128, 10]]
...
# sample = sample.loc[~(df_hold.sum(axis=1) == 0)].reset_index(drop=True)
## %% --------------------------------------------------------------------------------
x_train = np.array(df)
...
df = pd.DataFrame(np.vstack(fcs_list), columns=ff.pnn_labels)
sample_pred = pd.Series(sample_pred)
excludedPro = ['b-Amyloid_X40', 'b-Amyloid_X42', 'p-Tau', 'a-Synuclein_pS129',
                'EAAT1', 'GFAP', 'Casp3_Acti', '3NT', 'LC3B', 'K48-Ubiquitin']
            #    'PARKIN', 'TMEM230_C20orf30', 'DJ-1_PARK7', 'GBA1'] #possible
df = df.loc[:, ~df.columns.isin(excludedPro)]
...
# df_hold = df.drop(['SNAP25', 'CD56'], axis=1)
# df = df.loc[~(df_hold.sum(axis=1) == 0), :].reset_index(drop=True)
# sample_pred = sample_pred.loc[~(df_hold.sum(axis=1) == 0)].reset_index(drop=True)
## %% --------------------------------------------------------------------------------
x_train = np.array(df)
# scaler = StandardScaler(with_mean=False)
# x_train = scaler.fit_transform(x_train)
...
to_R = pd.concat([cl_pred, pd.DataFrame(sample_pred).rename(columns={0:'sample'})], axis=1)
